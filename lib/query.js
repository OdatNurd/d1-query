...
export function dbPrepareStatements(db, ...sqlargs) {
  ...
  for (const arg of sqlargs) {
    ...
    if (isBindValue) {
      ...
      statements.push(new SQLStatement(lastStatement.statement.bind(...orderedBinds), lastStatement.bindMetadata));
      pushed = true;
      ...
      if (typeof arg === "string") {
        const { sql, bindMetadata } = processSQLString(arg, false);
        newStatement = new SQLStatement(db.prepare(sql), bindMetadata);
