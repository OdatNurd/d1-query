 ...
 * either anonymous, in which case there are just unnamed `?` parameters, or it
 * is named, in which case parameters are specified by name. */
...
function processSingleAST(ast) {
  ...
  walkAST(ast, (node) => {
    ...
    if (node.type === 'param' && node.value !== null) {
      ...
      paramStyle = ':';
    } else if (node.type === 'var' && node.prefix === '$') {
      paramName = node.name;
      paramStyle = '$';
    } else if (node.type === 'var' && node.prefix === '@') {
      throw new SQLSyntaxError(`D1 does not support T-SQL style variables (@${node.name})`);
    }
  ...
  const newSql = sqlParser.sqlify(ast, { database: 'SQLite' });
...
export function processSQLString(sql, allowMultiple = false) {
  let ast;
  try {
    ast = sqlParser.astify(sql, { database: 'SQLite' });
...
export function mapBinds(metadata, values) {
  ...
  // anonymous bind style; otherwise, the array that we got can be directly used
  // in the bind, after we validate it a little further down.
  if (Array.isArray(values)) {
    if (metadata.style !== BIND_STYLE_ANONYMOUS) {
      throw new SQLBindError('an array of bind values can only be used with anonymous (?) parameters');
    }
    orderedParams = values;

  // Objects are allowed for binds, but only for statements that have named
  // bind parameters, which for our purposes is anything that's not anonymous
  // (since the parsing library does not directly support the ?# style).
