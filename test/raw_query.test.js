...
export default Collection`Raw Data Queries`({
  ...
  "Raw Query Failures": async ({ runScope: ctx}) => {
    ...
    await $check`SELECT with an undefined statement`
      .value(dbRawQuery(ctx.env.DB, undefined, 'raw_fail_test_one'))
