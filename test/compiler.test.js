...
export default Collection`Statement Compilation`({
  ...
  "Statement Preparation": ({ runScope: ctx }) => {
    ...
    $check`Single statement, no binds`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', 'SELECT * FROM Users WHERE userId = 1;'))
      .neq($, undefined)
      .isNotArray();
    ...
    $check`Single statement, two binds`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', 'SELECT * FROM Users WHERE userId = ?;', [1], [69]))
      .isArray()
      .eq($.length, 2);
  ...
  "Statement Re-Use": ({ runScope: ctx }) => {
    ...
    const stmt = dbPrepareStatements(ctx.env.DB, 'test_action', 'SELECT * FROM Users WHERE userId = 1;');
    const boundStmt = dbPrepareStatements(ctx.env.DB, 'test_action', 'SELECT * FROM Users WHERE userId = ?;');

    // A single statement should compile to a single object.
    $check`Single statement, no binds`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', stmt))
      .neq($, undefined)
      .isNotArray();

    // A single statement should compile to a single object, even with a bind.
    $check`Single statement, one bind`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', boundStmt, [1]))
      .neq($, undefined)
      .isNotArray();

    // A single statement with two binds should result in two statements.
    $check`Single statement, two binds`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', boundStmt, [1], [69]))
      .isArray()
      .eq($.length, 2);

    // The same should happen if it's just two statements.
    $check`Two statements, no binds`
      .value(dbPrepareStatements(ctx.env.DB, 'test_action', stmt, stmt))
      .isArray()
      .eq($.length, 2);
  ...
  "Statement Failures": ({ runScope: ctx }) => {
    ...
    $check`No arguments`
      .call(() => dbPrepareStatements(ctx.env.DB, 'test_action'))
    ...
    $check`Bind args before statements`
      .call(() => dbPrepareStatements(ctx.env.DB, 'test_action', [69], boundStmt))
    ...
    $check`Function arg is not SQL,pre-compiled statement or bind args`
      .call(() => dbPrepareStatements(ctx.env.DB, 'test_action', 69))
