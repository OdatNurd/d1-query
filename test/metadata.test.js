...
export default Collection`Statement Preparation`({
  ...
  "SQL Processing": ({ runScope: ctx }) => {
    ...
    // Using T-SQL style variables should throw an error.
    $check`T-SQL style variables are not allowed`
      .call(() => processSQLString('SELECT * FROM Users WHERE userId = @id;'))
      .throws($, 'D1 does not support T-SQL style variables (@id)');
  ...
  "Bind Mapping": ({ runScope: ctx }) => {
    ...
    $check`array for named query`
      ...
      .throws($, 'an array of bind values can only be used with anonymous (?) parameters');
